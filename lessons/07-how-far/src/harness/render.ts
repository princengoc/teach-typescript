...
// One still frame: ghost target, painted cells, robot.
export function drawWorld(options: FrameOptions): void {
  const { canvas, world, target } = options;
...
export function replay(options: ReplayOptions): () => void {
  const { canvas, start, commands, target } = options;
