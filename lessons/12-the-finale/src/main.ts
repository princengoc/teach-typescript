...
import {
  ...
  pictureVariants,
  randomBoxVariant,
  randomDiamondVariant,
  randomPictureVariant,
  startWorld,
  targetWorld,
  toRoom,
  type Variant,
...
// The intro runs on the first room rung 3 is graded in, so the picture the kid
// watches the walk uncover is the picture their own walk has to draw.
const FIRST_ROOM = pictureVariants[0];
if (!FIRST_ROOM) throw new Error('lesson 12: a picture room is required');
...
// The intro animation walks the whole box, a square at a time. The left column
// is the rule's answer about the square under the robot; the right column is
// what the robot did about it.
function runDemoAnim(): void {
  ...
  const lines = [`${column('the rule says')}what the robot does`, ''];
  let index = 0;
  let spoken = '';
  let timer = 0;
  ...
  const tick = (): void => {
    ...
    if (!beat) {
      lines.push('');
      lines.push(`${squaresAsked(script)} squares. One walk. One rule.`);
      log.textContent = lines.join('\n');
      return;
    }
...
function drawDemoRest(): void {
  ...
  const log = el('demo-log');
  if (log) {
    log.textContent = 'Press the button: one square at a time.';
  }
}
...
function setIntroText(): void {
  ...
  const slots: [string, string][] = [
    ['data-intro-room', INTRO.label],
    ['data-intro-squares', String(squaresAsked(script))],
    ['data-intro-marks', INTRO.room.marks.join(', ')],
  ];
...
const RUNGS: Rung[] = [
  {
    key: 'box',
    title: '1. Walk the whole box',
    program: paintBox,
    variants: boxVariants,
    mystery: randomBoxVariant,
    checks: [],
  },
  {
    key: 'diamond',
    title: '2. A rule made of arithmetic',
    program: paintDiamond,
    variants: diamondVariants,
    mystery: randomDiamondVariant,
    checks: [
      { label: 'gap', run: () => checkGap(gap) },
      { label: 'near', run: () => checkRule(near, diamondVariants) },
    ],
  },
  {
    key: 'picture',
    title: '3. A rule made of data, and the whole walk',
    program: paintPicture,
    variants: pictureVariants,
    mystery: randomPictureVariant,
    checks: [
      { label: 'cellNumber', run: () => checkCellNumber(cellNumber) },
      { label: 'wanted', run: () => checkRule(wanted, pictureVariants) },
    ],
  },
];
...
// the walk around it is still wrong.
